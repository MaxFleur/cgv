...
*/
class bpcp_overlay : public cgv::glutil::overlay {
protected:
	...
	cgv::glutil::generic_renderer polygon_renderer;
...
public:
	bpcp_overlay();
	std::string get_type_name() const { return "bpcp_overlay"; }
	...
	void set_data(std::vector<float>& minmax_data, std::vector<float>& volume_data) {
		this->minmax_data = minmax_data;
		this->volume_data = volume_data;
	}
};

typedef cgv::data::ref_ptr<bpcp_overlay> bpcp_overlay_ptr;
