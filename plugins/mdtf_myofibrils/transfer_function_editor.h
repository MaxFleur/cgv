...
class transfer_function : public cgv::render::render_types {
public:
...
class transfer_function_editor :
...
protected:
	...
	struct point {
		...
		void update_pos(const layout_attributes& la) {
			...
			val.x() = cgv::math::clamp(val.x(), 0.0f, 1.0f);
			val.y() = cgv::math::clamp(val.y(), 0.0f, 1.0f);
			pos = la.editor_rect.pos() + val * la.editor_rect.size() - radius;
		}
	...
	std::vector<tf_container> containers;
	unsigned container_idx;
	tf_container* current_container;
	std::string names[4] = { "Projectin", "Actin", "Obscurin", "Sallimus" };
...
public:
	...
	void set_histogram(unsigned idx, const std::vector<unsigned>& data);
