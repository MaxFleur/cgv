...
#else
		std::string base_path = "C:\\Uni\\Masterarbeit\\cgv\\plugins\\mdtf_myofibrils";
...
bool viewer::read_transfer_functions(context& ctx, const std::string& filename) {
	...
	if(cgv::glutil::color_map_reader::read_from_xml(filename, names, color_maps)) {
		if(names.size() == color_maps.size() || color_maps.size() < 4) {
