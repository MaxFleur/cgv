...
void sp_overlay::draw_content(cgv::render::context& ctx) {
	...
	if(total_count + count > points.get_vertex_count())
		count = points.get_vertex_count() - total_count;
	...
	bool run = total_count < points.get_vertex_count();
	if(run)
		post_redraw();
	else
...
void sp_overlay::update_content() {
	...
	std::cout << "Drawing " << points.get_vertex_count() << " points" << std::endl;
