...
namespace cgv { // @<
	namespace render { // @<
		...
		extern special_slab_renderer& ref_special_slab_renderer(context& ctx, int ref_count_change = 0);
		...
		*/
		struct special_slab_render_style : public group_render_style
		{
			enum Mode {
				COLOR = 0,
				TRANSFER_FUNCTION = 1,
			...
			// TODO: move tex units out of style
			...
			special_slab_render_style();

			void on_set_clip_plane(cgv::base::base* base_ptr, const vec3& v) {
				clip_plane_normal = v;
				base_ptr->on_set(&clip_plane_normal[0]);
				base_ptr->on_set(&clip_plane_normal[1]);
				base_ptr->on_set(&clip_plane_normal[2]);
			}

			void flip_clip_plane(cgv::base::base* base_ptr) {
				on_set_clip_plane(base_ptr, -clip_plane_normal);
			}

			void clip_plane_normal_x(cgv::base::base* base_ptr) {
				on_set_clip_plane(base_ptr, vec3(1.0f, 0.0f, 0.0f));
			}

			void clip_plane_normal_y(cgv::base::base* base_ptr) {
				on_set_clip_plane(base_ptr, vec3(0.0f, 1.0f, 0.0f));
			}

			void clip_plane_normal_z(cgv::base::base* base_ptr) {
				on_set_clip_plane(base_ptr, vec3(0.0f, 0.0f, 1.0f));
			}
		};

		/// renderer that supports point splatting
		class special_slab_renderer : public group_renderer
		{
		protected:
		...
		struct special_slab_render_style_reflect : public special_slab_render_style
		{
			bool self_reflect(cgv::reflect::reflection_handler& rh);
		};
		extern cgv::reflect::extern_reflection_traits<special_slab_render_style, special_slab_render_style_reflect> get_reflection_traits(const special_slab_render_style&);
	}
}
