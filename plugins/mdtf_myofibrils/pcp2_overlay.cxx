...
void pcp2_overlay::draw_content(cgv::render::context& ctx) {
	...
	if(total_count + count > lines.get_vertex_count())
		count = lines.get_vertex_count() - total_count;
	...
	bool run = total_count < lines.get_vertex_count();
	if(run)
		post_redraw();
	else
...
void pcp2_overlay::update_content() {
	...
	std::cout << "Drawing " << lines.get_vertex_count() / 2 << " lines" << std::endl;
